...
var maxSubArray = function (nums) {
    ...
    //     } 
    //     else {
    //         if(nums[0] > nums[1] + nums[1] && nums[0] > nums[])
    //     }
    // }
    let subArrs = [];
    let sumsOfSubArrs = [];
    ...
    for (let i = 0; i < nums.length; i++) {
        ...
        for(let j = i+1; j <= nums.length; j++) {
            tempArrs.push(nums.slice(i,j));
        }
        for(let k = 0 ; k < tempArrs.length; k++) {
            let sum = 0;
            tempArrs[k].forEach(ele => sum += ele);
            tempSumArrs[k] = tempSumArrs;
        }
        let currMax = Math.max(...tempSumArrs);
        sumsOfSubArrs.push(currMax);
    }
